 ...
 * body: { localProjectId, projectName, repo, accessToken }
 *
 * 연동 등록 + 동기화를 한 번에 처리한다. 백엔드는 실제 GitHub API로 기본 브랜치의
...
export async function POST(req: NextRequest) {
  let body: { localProjectId?: string; projectName?: string; repo?: string; accessToken?: string };
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "요청 바디가 JSON이 아닙니다." }, { status: 400 });
  }
  if (!body.localProjectId || !body.projectName || !body.repo || !body.accessToken) {
    return NextResponse.json(
      { error: "localProjectId, projectName, repo, accessToken이 모두 필요합니다." },
      { status: 400 }
    );
  }
