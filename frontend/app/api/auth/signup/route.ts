import { NextRequest, NextResponse } from "next/server";
import { setSessionCookie } from "@/lib/session";

export const runtime = "nodejs";

/**
 * POST /api/auth/signup — 백엔드 POST /auth/signup을 그대로 프록시한다.
 * 회원가입도 permitAll 엔드포인트라 고정 서비스 토큰 없이 호출한다.
 */
export async function POST(req: NextRequest) {
  const body = await req.json().catch(() => null);
  if (!body?.email || !body?.password || !body?.name) {
    return NextResponse.json({ error: "이름, 이메일, 비밀번호를 모두 입력해주세요." }, { status: 400 });
  }

  const baseUrl = process.env.BACKEND_BASE_URL;
  if (!baseUrl) {
    return NextResponse.json({ error: "BACKEND_BASE_URL이 서버에 설정되어 있지 않습니다." }, { status: 500 });
  }

  try {
    const backendRes = await fetch(`${baseUrl.replace(/\/$/, "")}/auth/signup`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: body.email, password: body.password, name: body.name }),
      cache: "no-store",
    });
    const data = await backendRes.json().catch(() => ({}));
    if (!backendRes.ok || !data.success) {
      return NextResponse.json(
        { error: data.message ?? `회원가입에 실패했습니다 (HTTP ${backendRes.status})` },
        { status: backendRes.status }
      );
    }
    const res = NextResponse.json({ token: data.data.token, user: data.data.user });
    setSessionCookie(res, data.data.token);
    return res;
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return NextResponse.json({ error: message }, { status: 502 });
  }
}
